// autoresearch v88: Fine-tune F1=95.9%
const fs = require('fs');
...
// Ultra-fine around v87 best
console.log('=== Ultra-fine combined ===\n');
{
  let best={f1:0}, count=0;
  
  for(let dW=1.5;dW<=3;dW+=0.125){
    for(let dEntTh=0.625;dEntTh<=0.875;dEntTh+=0.0625){
      const votes = all.map((s,i) => {
        let v = v85votes(s, i);
        if(!isHighJW[i] && dtEnt5[i]<dEntTh) v -= dW;
        return v;
      });
      const s1pred = votes.map(v => v >= 4);
      
      for(let hwH=4;hwH<=8;hwH+=2){
        for(let nThH=0.15;nThH<=0.35;nThH+=0.05){
          for(let hwL=8;hwL<=12;hwL+=2){
            for(let nThL=0.5;nThL<=0.7;nThL+=0.05){
              for(let lowH=-5;lowH<=-3;lowH+=1){
                for(let lowL=-2;lowL<=0;lowL+=0.5){
                  ...
                  if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
                    best={...r,dW,dEntTh,hwH,nThH,hwL,nThL,lowH,lowL};
                    count++;
                  }
                }
              }
...
console.log('v85: F1=94.4%');
console.log('v87: F1=95.9%');
