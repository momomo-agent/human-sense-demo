// autoresearch v85: Fine-tune F1=94.4% — p1 + burstLen combo
const fs = require('fs');
...
// ============================================================
// Part 1: Ultra-fine p1 + burstLen
// ============================================================
console.log('=== Part 1: Ultra-fine p1 + burstLen ===\n');
{
  let best={f1:0}, count=0;
  
  for(let p1W=1;p1W<=4;p1W+=0.125){
    for(let p1VelTh=0.25;p1VelTh<=0.6;p1VelTh+=0.025){
      for(let p1ScoreTh=0.3;p1ScoreTh<=0.5;p1ScoreTh+=0.025){
        for(let blW=0.5;blW<=3;blW+=0.125){
          for(let blTh=1;blTh<=3;blTh++){
            const votes = all.map((s,i) => {
              let v = v82votes(s, i);
              if(!isHighJW[i] && dt[i]<0.001 && s.jawVelocity>=p1VelTh && s.score<p1ScoreTh) v -= p1W;
              if(!isHighJW[i] && burstLen[i]<=blTh && dt[i]<0.001) v -= blW;
              return v;
            });
            ...
            if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
              best={...r,p1W,p1VelTh,p1ScoreTh,blW,blTh};
              count++;
            }
          }
        }
  ...
  if(best.f1>0){
    ...
    console.log(`  burst: W=${best.blW} len<=${best.blTh}`);
  }
}

// ============================================================
// Part 2: Also add dt>0 specific penalties for remaining FP
// ============================================================
console.log('\n=== Part 2: + dt>0 penalties ===\n');
{
  let best={f1:0}, count=0;
  
  // Use best p1+burst from v84, add dt>0 penalties
  for(let p1W=1.5;p1W<=3;p1W+=0.5){
    for(let p1VelTh=0.35;p1VelTh<=0.5;p1VelTh+=0.05){
      for(let p1ScoreTh=0.35;p1ScoreTh<=0.45;p1ScoreTh+=0.05){
        for(let blW=1;blW<=2.5;blW+=0.25){
          // dt>0 + jw=0.2 + medium score + low gap
          for(let dtPosW=0;dtPosW<=2;dtPosW+=0.5){
            for(let dtPosScoreLow=0.5;dtPosScoreLow<=0.7;dtPosScoreLow+=0.1){
              for(let dtPosGapTh=0.05;dtPosGapTh<=0.1;dtPosGapTh+=0.025){
                const votes = all.map((s,i) => {
                  let v = v82votes(s, i);
                  if(!isHighJW[i] && dt[i]<0.001 && s.jawVelocity>=p1VelTh && s.score<p1ScoreTh) v -= p1W;
                  if(!isHighJW[i] && burstLen[i]<=2 && dt[i]<0.001) v -= blW;
                  if(dtPosW>0 && !isHighJW[i] && dt[i]>=0.001 && s.score>=dtPosScoreLow && s.score<0.75 && scoreGap[i]<dtPosGapTh) v -= dtPosW;
                  return v;
                });
                ...
                const r=ev(preds);
                if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
                  best={...r,p1W,p1VelTh,p1ScoreTh,blW,dtPosW,dtPosScoreLow,dtPosGapTh};
                  count++;
                }
              }
            }
  ...
  console.log(`+dt>0: ${count} qualifying`);
  if(best.f1>0){
    console.log(`Best: R=${(best.recall*100).toFixed(1)}% S=${(best.specificity*100).toFixed(1)}% F1=${(best.f1*100).toFixed(1)}% FP=${best.FP} FN=${best.FN}`);
    console.log(`  p1: W=${best.p1W} vel>=${best.p1VelTh} score<${best.p1ScoreTh}`);
    console.log(`  burst: W=${best.blW}`);
    console.log(`  dt>0: W=${best.dtPosW} score>=${best.dtPosScoreLow} gap<${best.dtPosGapTh}`);
  }
}
...
console.log('v81: F1=91.9%');
console.log('v84: F1=94.4% (p1+burstLen)');
