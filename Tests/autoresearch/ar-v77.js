// autoresearch v77: jawWeight binary feature + combined optimization
// jawWeight=0.2 → 90.4% AI, jawWeight=1.0 → 63.2% user
// This is a BINARY feature — strongest discriminator we have!
const fs = require('fs');
...
const jwMean5 = wstat(isHighJW.map(v=>v?1:0), 2, mean);
...
// ============================================================
// Part 1: jawWeight as penalty + bonus with full parameter search
// ============================================================
console.log('=== Part 1: jawWeight full search ===\n');
{
  let best={f1:0}, count=0;
  
  // jawWeight=0.2 (low) → penalty
  // jawWeight=1.0 (high) → bonus
  for(let lowPen=0;lowPen<=3;lowPen+=0.125){
    for(let highBon=0;highBon<=2;highBon+=0.125){
      for(let t=3.5;t<=5;t+=0.25){
        const votes = all.map((s,i) => {
          let v = v73sc[i];
          if(!isHighJW[i]) v -= lowPen;
          if(isHighJW[i]) v += highBon;
          return v;
        });
        
        const s1pred = votes.map(v => v >= t);
        // Two-pass with hw=13 nTh=0.7 (from v76 best)
        const preds = all.map((_,i) => {
          if(s1pred[i]) return true;
          if(votes[i] < -3) return false;
        ...
        if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
          best={...r,lowPen,highBon,t};
          count++;
        }
      }
    }
  }
  console.log(`jawWeight full: ${count} qualifying`);
  if(best.f1>0){
    ...
    console.log(`  lowPen=${best.lowPen} highBon=${best.highBon} t=${best.t}`);
  }
}

// ============================================================
// Part 2: jawWeight + full rescue re-optimization
// ============================================================
console.log('\n=== Part 2: jawWeight + rescue full search ===\n');
{
  let best={f1:0}, count=0;
  
  for(let lowPen=0;lowPen<=2;lowPen+=0.25){
    for(let highBon=0;highBon<=1.5;highBon+=0.25){
      for(let t=3.5;t<=5;t+=0.25){
        for(let rLow=-3;rLow<=0;rLow+=1){
          for(let rHw=10;rHw<=14;rHw+=1){
            for(let rNTh=0.6;rNTh<=0.8;rNTh+=0.05){
              ...
              const r=ev(preds);
              if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
                best={...r,lowPen,highBon,t,rLow,rHw,rNTh};
                count++;
              }
            }
          }
...
// ============================================================
// Part 3: jawWeight window feature (neighborhood jawWeight density)
// ============================================================
console.log('\n=== Part 3: jawWeight neighborhood ===\n');
{
  let best={f1:0};
  
  for(let lowPen=0;lowPen<=2;lowPen+=0.25){
    for(let highBon=0;highBon<=1;highBon+=0.25){
      // Also: penalize if neighborhood has low jawWeight density
      for(let jwDensW=0;jwDensW<=2;jwDensW+=0.25){
        for(let jwDensTh=0.2;jwDensTh<=0.5;jwDensTh+=0.1){
          const votes = all.map((s,i) => {
            let v = v73sc[i];
            if(!isHighJW[i]) v -= lowPen;
            if(isHighJW[i]) v += highBon;
            if(jwDensW>0 && jwMean5[i]<jwDensTh) v -= jwDensW;
            return v;
          });
          
          for(let t=3.5;t<=5;t+=0.5){
            const s1pred = votes.map(v => v >= t);
            const preds = all.map((_,i) => {
              if(s1pred[i]) return true;
              if(votes[i] < -3) return false;
            ...
            const r=ev(preds);
            if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
              best={...r,lowPen,highBon,jwDensW,jwDensTh,t};
            }
          }
        }
      }
  ...
  if(best.f1>0){
    console.log(`jwNeighborhood: R=${(best.recall*100).toFixed(1)}% S=${(best.specificity*100).toFixed(1)}% F1=${(best.f1*100).toFixed(1)}% FP=${best.FP} FN=${best.FN}`);
    console.log(`  lowPen=${best.lowPen} highBon=${best.highBon} jwDensW=${best.jwDensW} jwDensTh=${best.jwDensTh} t=${best.t}`);
  }
}

console.log('\n=== PROGRESS ===');
console.log('v73: F1=89.1%, FP=42, FN=9');
console.log('v76: F1=89.5%, FP=40, FN=9 (rescue re-tune)');
