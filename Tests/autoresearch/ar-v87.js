// autoresearch v87: Combine dtEnt5 penalty + aggressive rescue
// Part 2 best: dtEnt5<0.75 → -2.0 (F1=95.0%, FP=12, FN=10)
// Part 3 best: aggressive rescue (F1=94.9%, FP=17, FN=6)
// Combine: penalty reduces FP, rescue reduces FN
const fs = require('fs');
...
// ============================================================
// Combined: dtEnt5 penalty + aggressive rescue
// ============================================================
console.log('=== Combined: dtEnt5 penalty + rescue ===\n');
{
  let best={f1:0}, count=0;
  
  for(let dW=0;dW<=3;dW+=0.25){
    for(let dEntTh=0.5;dEntTh<=1;dEntTh+=0.125){
      ...
      for(let hwH=4;hwH<=10;hwH+=2){
        for(let nThH=0.2;nThH<=0.6;nThH+=0.1){
          for(let hwL=6;hwL<=12;hwL+=2){
            for(let nThL=0.5;nThL<=0.9;nThL+=0.1){
              for(let velTh=0.075;velTh<=0.125;velTh+=0.025){
                for(let lowH=-5;lowH<=-2;lowH+=1){
                  for(let lowL=-3;lowL<=0;lowL+=1){
                    ...
                    if(r.recall>=0.9&&r.specificity>=0.9&&r.f1>best.f1){
                      best={...r,dW,dEntTh,hwH,nThH,hwL,nThL,velTh,lowH,lowL};
                      count++;
                    }
                  }
                }
  ...
  if(best.f1>0){
    ...
    console.log(`  dtEnt5: W=${best.dW} th<${best.dEntTh}`);
